...
class ScheduleAdapter(
    ...
    inner class ViewHolder(private val b: ItemScheduleBinding) : RecyclerView.ViewHolder(b.root) {
        fun bind(s: Schedule) {
            ...
            val subject = if (s.subjectId.isNotBlank()) {
                StudyRepository.getInstance(ctx).getSubjectById(s.subjectId)
            } else null

            if (subject != null && subject.colorHex.isNotBlank()) {
                try {
                    b.subjectColorBar.setBackgroundColor(Color.parseColor(subject.colorHex))
                    b.subjectColorBar.visibility = View.VISIBLE
                } catch (e: Exception) {
                    b.subjectColorBar.visibility = View.GONE
                }
            } else {
                b.subjectColorBar.visibility = View.GONE
            }
